...
__private dt_pointer idt_pointer;
__private gate idt[0x0100];
...
__public void exception_handler_bus(dword vector, interrupt_stack_frame* stack_frame)
{
	exception_handler handler = exception_handlers[vector];
	if(handler != 0) handler(vector, stack_frame);
}
...
__private void default_exception_handler(dword vector, interrupt_stack_frame* stack_frame)
{
	if(vector < 20)
	{
		...
		video_put_string("\nip: 0x", 0x07);
		video_put_string(convert_hex(stack_frame->eip, 8), 0x07);

		video_put_string("\ncs: 0x", 0x07);
		video_put_string(convert_hex(stack_frame->cs & 0xfff8, 4), 0x07);
		video_put_string(" (privilege: ", 0x07);
		video_put_string(convert_hex(stack_frame->cs & 0x0003, 1), 0x07);
		video_put_string(")", 0x07);

		video_put_string("\neflags: ", 0x07);
		video_put_string(convert_hex(stack_frame->eflags, 8), 0x07);

		if(stack_frame->error_code != 0xffffffff)
		{
			video_put_string("\nerror_code: ", 0x07);
			video_put_string(convert_hex(stack_frame->error_code, 8), 0x07);
		}
...
extern void asm_default_interrupt_handler(interrupt_stack_frame*);
...
extern void asm_interrupt_set_idt(dt_pointer* idt_pointer);
extern void asm_interrupt_save_dataregs();
extern selector asm_interrupt_get_cs();
...
#define __interrupt_gate_setup(index, tag) gate_new(&idt[index], tag, interrupt_code_selector, \
		...
		privilege_kernel);
__public void interrupt_idt_initialize()
{
	idt_pointer.base = idt;
	idt_pointer.limit = sizeof(idt) - 1;

	asm_interrupt_set_idt(&idt_pointer);
	asm_interrupt_save_dataregs();

	interrupt_code_selector = asm_interrupt_get_cs();
	int i = 0;
	for(; i < 20; i ++)
